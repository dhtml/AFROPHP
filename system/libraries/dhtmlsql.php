...
class DHTMLSQL {
	...
	*/
    ...
    $replace=Array();
      foreach($matches[1] as $table) {
        $search[]='{'.$table.'}';
        $replace[]="{$table_prefix}{$table}";
      }
     $stmt=str_replace($search,$replace,$stmt);
    }
    return $stmt;
		//return strtr($stmt, array('{' => $table_prefix, '}' => ''));
	}



  /**
  * Adds the dbprefix to a tablename (if the table does not contain the prefix)
  *
  * @param	string table   The name of the table e.g. user
  * @return	string
  */
  public function add_prefix($table) {
      if(empty(dbprefix)) {return $table;}
      if(strpos($table, dbprefix) !== 0) {$table=dbprefix.$table;}
      return $table;
  }
     ...
     *  <code>
     *	$db=DHTMLSQL::_get()->connect('localhost','admin','pass','dbtest');
     *  </code>
	 *
     *  @return object		DHTMLSQL class object is returned
     *
	*/
	public static function _get() {return self::load();}
	...
	public static function connect($host = null,$username = null,$password = null,$dbname = null,$port=null,$socket=null) {
		return self::_get()->_connect($host,$username,$password,$dbname,$port,$socket);
	}
	...
	*/
	public function preview($mode) {
      ...
      /**
       *  sets/gets the current database prefix prefix
       *
       *
       *  <code>
       *	 $db->prefix('ow_');
    ...
    //prefix tables with {table}
    $sql=$this->db_prefix_tables($sql,$this->prefix);
  ...
  /**
    ...
    *
    *
    *  @param  int  $length           (Optional) The length of data to return
    *
    *  @param  int  $start           (Optional) The starting point of the query
    *
    *  @param  string  $where           (Optional) The where aspect of the query
    *
    *  @param  string  $suffix           (Optional) Any other addition to the query
    *
    *
    *  @return array                  Associative array of the response
    *
    */
 function get($table,$length=0,$start=0,$where='',$suffix='')
 {
   ...
   return $this->query(trim($query.' '.$suffix))->fetch_assoc_all();
 }
  ...
  *
	*/
	function insert_json_url($table,$url)
	{
  ...
  *
	*/
	function insert_json_string($table,$json)
	{
...
*  benchmark
*
*  Run benchmark tests on database, checks cache and all that
*
* @return void
*/
public function benchmark() {
@header("Content-type:text/plain");
$table="dhtmlsql_benchmark";
print<<<end
Preparing to run mysqli benchmark tests
This test will use $table table

end;

$result=$this->query("show variables like 'query%';")->fetch_assoc_all();
$status=Array();
foreach($result as $item) {
extract($item);
$status[$Variable_name]=$Value;
}
echo "\nHere is the status of your query variables
...
echo "should be similar to:\n";
print_r($expected);


if($status['query_cache_type']!='ON') {
print<<<end

To enable database cache, add the following to your mysql config:
...
$result=$this->query("SHOW STATUS LIKE 'Qc%';")->fetch_assoc_all();
$status=Array();
foreach($result as $item) {
extract($item);
$status[$Variable_name]=$Value;
...
$mysqli->query("CREATE TABLE $table(id INT)");
$mysqli->query("INSERT INTO $table(id) VALUES (1), (2)");

/* Will be cached because of the SQL hint */
$start = microtime(true);
$res   = $mysqli->query("SELECT * FROM $table WHERE id = 1");

echo "\n\nBenchark SQL Queries:\n";
...
/* No cache hit */
$start = microtime(true);
$res   = $mysqli->query("SELECT SQL_NO_CACHE * FROM $table WHERE id = 1");
...
echo "\n\nExplanation of Symbols (assuming query cache is working):\n";
echo "Q1: Total time of first query (runs and stores in cache) - so should take the longest time\n";
echo "Q2: Total time to fetch query from cache (should take the shortest time)\n";
echo "Q3: Total time to fetch uncached query (should be shorter than Q1 but longer than Q2)\n";
...
--
	...
	*/
    ...
    if(function_exists('show_error')) {show_error($message,'Database Error',2);}
